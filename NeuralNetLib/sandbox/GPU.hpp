...
inline bool initOpenCL() {
	...
	if (devices.empty()) {
		...
		#ifdef OPEN_CL_CPU_DEBUG
			newP.getDevices(CL_DEVICE_TYPE_CPU, &devices);

			if (devices.empty())
				std::cerr << "No OpenCL CPU/GPU devices found." << std::endl;
			else
			 	std::cerr << "No OpenCL GPU devices found. Falling back to CPU." << std::endl;
